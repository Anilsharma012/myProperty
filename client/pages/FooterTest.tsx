  ...
  const testIndividualEndpoints = async () => {
    ...
    try {
      ...
      const settingsResponse = await fetch(
        createApiUrl("/api/footer/settings"),
      );
              ...
              {testData && (
                <div className="space-y-2">
                  ...
                  <div className="text-sm space-y-1">
                    <p>
                      <strong>Total Links:</strong> {testData.stats.totalLinks}
                    </p>
                    <p>
                      <strong>Active Links:</strong>{" "}
                      {testData.stats.activeLinks}
                    </p>
                    <p>
                      <strong>Has Settings:</strong>{" "}
                      {testData.stats.hasSettings ? "Yes" : "No"}
                    </p>
                    <p>
                      <strong>Last Updated:</strong>{" "}
                      {new Date(testData.stats.lastUpdated).toLocaleString()}
                    </p>
                  </div>
                </div>
              )}
            ...
            <CardContent>
              {testData?.settings && (
                <div className="space-y-2 text-sm">
                  <p>
                    <strong>Company:</strong> {testData.settings.companyName}
                  </p>
                  <p>
                    <strong>Logo:</strong> {testData.settings.companyLogo}
                  </p>
                  <p>
                    <strong>Description:</strong>{" "}
                    {testData.settings.companyDescription?.substring(0, 100)}...
                  </p>
                  <p>
                    <strong>Show Locations:</strong>{" "}
                    {testData.settings.showLocations ? "Yes" : "No"}
                  </p>
                  <p>
                    <strong>Locations:</strong>{" "}
                    {testData.settings.locations?.join(", ")}
                  </p>

                  <div className="mt-4">
                    <p className="font-semibold">Social Links:</p>
                    <ul className="ml-4 space-y-1">
                      {testData.settings.socialLinks?.facebook && (
                        <li>✓ Facebook</li>
                      )}
                      {testData.settings.socialLinks?.twitter && (
                        <li>✓ Twitter</li>
                      )}
                      {testData.settings.socialLinks?.instagram && (
                        <li>✓ Instagram</li>
                      )}
                      {testData.settings.socialLinks?.youtube && (
                        <li>✓ YouTube</li>
                      )}
                    </ul>
                  </div>
                  ...
                  <div className="mt-4">
                    ...
                    <ul className="ml-4 space-y-1">
                      {testData.settings.contactInfo?.phone && (
                        <li>📞 {testData.settings.contactInfo.phone}</li>
                      )}
                      {testData.settings.contactInfo?.email && (
                        <li>✉️ {testData.settings.contactInfo.email}</li>
                      )}
                      {testData.settings.contactInfo?.address && (
                        <li>📍 {testData.settings.contactInfo.address}</li>
                      )}
                    </ul>
                  </div>
                </div>
            ...
            <CardContent>
              {testData?.links && testData.links.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {["quick_links", "legal", "company", "support"].map(
                    (section) => {
                      const sectionLinks = testData.links.filter(
